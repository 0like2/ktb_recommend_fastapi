from methods.evaluation_methods import volume_evaluation, content_evaluation, regulation_evaluation
from methods.generation_methods import content_feedback, regulation_feedback, summary_generator

from prompts.generation_prompt import GenerationPrompt
from prompts.evaluation_prompt import EvaluationPrompt

from schemas import ProposalEvaluationRequestDto, ProposalEvaluationResponseDto, SummaryGenerationRequestDto, \
    FeedbackDto, SummaryDto
...
def proposal_evaluation(request: ProposalEvaluationRequestDto):
    ...
    total_score = float(content_evaluation_result['message']) + float(content_evaluation_result['target']) + float(
        content_evaluation_result['relevance'])
    evaluated_proposal = "Message: " + content_evaluation_result['message'] + "Target: " + content_evaluation_result[
        'target'] + "Relevance: " + content_evaluation_result['relevance'] + SEP + proposal
    if (total_score < 6.0):
        generated_feedback = content_feedback(content_feedback_prompt=content_feedback_prompt,
    ...
    # Regulation Check
    regulation_evaluation_result = regulation_evaluation(proposal=proposal,
    ...
    if (not appropriate):
        ...
        generated_feedback = regulation_feedback(regulation_feedback_prompt=regulation_feedback_prompt,
